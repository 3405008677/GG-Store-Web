...
首次运行前复制 `.env.example`，把 `NUXT_API_PROXY_TARGET` 设置为
`GG.Store.UserManage` 启动日志中的 HTTPS 地址并重启 Nuxt。该开发代理只转发
`/api/auth/**`，默认前端开发端口为 `2606`。`NUXT_PUBLIC_API_BASE` 应保持同源
路径 `/api`，不要填写后端的绝对地址。
...
开发代理只在非生产环境启用。生产环境应由同源网关按路径把 `/api/auth/**` 转发到
UserManage，并为后续 Catalog 等服务分别配置路由，同时完整透传 `Set-Cookie`。
...
当前包含框架基础、API 封装、国际化、登录/退出闭环和静态商城首页；商品、购物车、
订单等领域接口与真实交互仍待后续业务模块接入。
